...
export const DIFFS: Record<string, DiffLine[]> = {
	'src-tauri/src/git/watcher.rs': [
		...
		{ kind: 'del', oldLine: 14, newLine: null, text: 'use std::time::Duration;' },
		{ kind: 'add', oldLine: null, newLine: 14, text: 'use std::time::{Duration, Instant};' },
		{ kind: 'add', oldLine: null, newLine: 15, text: 'use std::collections::HashMap;' },
		...
		{ kind: 'del', oldLine: 42, newLine: null, text: '        let (tx, rx) = channel();' },
		{ kind: 'add', oldLine: null, newLine: 44, text: '        // Agents write in bursts; collapse them before we touch Git.' },
		{ kind: 'add', oldLine: null, newLine: 45, text: '        let (tx, rx) = channel();' },
		{ kind: 'add', oldLine: null, newLine: 46, text: '        let mut seen: HashMap<PathBuf, Instant> = HashMap::new();' },
		{ kind: 'add', oldLine: null, newLine: 47, text: '        let debounce = Duration::from_millis(120);' },
		...
		{ kind: 'del', oldLine: 47, newLine: null, text: '        for event in rx {' },
		{ kind: 'del', oldLine: 48, newLine: null, text: '            self.app.emit("fs:changed", event.paths)?;' },
		{ kind: 'add', oldLine: null, newLine: 52, text: '        for event in rx.iter().flatten() {' },
		{ kind: 'add', oldLine: null, newLine: 53, text: '            let now = Instant::now();' },
		{ kind: 'add', oldLine: null, newLine: 54, text: '            let fresh: Vec<PathBuf> = event' },
		{ kind: 'add', oldLine: null, newLine: 55, text: '                .paths' },
		{ kind: 'add', oldLine: null, newLine: 56, text: '                .into_iter()' },
		{ kind: 'add', oldLine: null, newLine: 57, text: '                .filter(|p| match seen.get(p) {' },
		{ kind: 'add', oldLine: null, newLine: 58, text: '                    Some(last) => now.duration_since(*last) > debounce,' },
		{ kind: 'add', oldLine: null, newLine: 59, text: '                    None => true,' },
		{ kind: 'add', oldLine: null, newLine: 60, text: '                })' },
		{ kind: 'add', oldLine: null, newLine: 61, text: '                .collect();' },
		{ kind: 'add', oldLine: null, newLine: 62, text: '' },
		{ kind: 'add', oldLine: null, newLine: 63, text: '            if fresh.is_empty() { continue; }' },
		{ kind: 'add', oldLine: null, newLine: 64, text: '            for p in &fresh { seen.insert(p.clone(), now); }' },
		{ kind: 'add', oldLine: null, newLine: 65, text: '            self.app.emit("fs:changed", &fresh)?;' },
		...
		{ kind: 'del', oldLine: 59, newLine: null, text: '        self.handles.remove(&tab);' },
		{ kind: 'add', oldLine: null, newLine: 76, text: '        if let Some(handle) = self.handles.remove(&tab) {' },
		{ kind: 'add', oldLine: null, newLine: 77, text: '            handle.stop();' },
		{ kind: 'add', oldLine: null, newLine: 78, text: '        }' },
	...
	'src-tauri/src/git/diff.rs': [
		...
		{ kind: 'add', oldLine: null, newLine: 1, text: 'use git2::{Diff, DiffOptions, Repository};' },
		{ kind: 'add', oldLine: null, newLine: 2, text: 'use serde::Serialize;' },
		{ kind: 'add', oldLine: null, newLine: 3, text: '' },
		{ kind: 'add', oldLine: null, newLine: 4, text: '#[derive(Serialize)]' },
		{ kind: 'add', oldLine: null, newLine: 5, text: 'pub struct FileDiff {' },
		{ kind: 'add', oldLine: null, newLine: 6, text: '    pub path: String,' },
		{ kind: 'add', oldLine: null, newLine: 7, text: '    pub hunks: Vec<Hunk>,' },
		{ kind: 'add', oldLine: null, newLine: 8, text: '    pub added: usize,' },
		{ kind: 'add', oldLine: null, newLine: 9, text: '    pub removed: usize,' },
		{ kind: 'add', oldLine: null, newLine: 10, text: '}' }
	],
	'src-tauri/src/git/mod.rs': [
		{ kind: 'hunk', range: '@@ -1,5 +1,7 @@', context: '' },
		{ kind: 'ctx', oldLine: 1, newLine: 1, text: 'pub mod blame;' },
		{ kind: 'add', oldLine: null, newLine: 2, text: 'pub mod diff;' },
		{ kind: 'ctx', oldLine: 2, newLine: 3, text: 'pub mod log;' },
		{ kind: 'del', oldLine: 3, newLine: null, text: 'pub mod watch;' },
		{ kind: 'add', oldLine: null, newLine: 4, text: 'pub mod watcher;' },
	...
	'src/lib/components/DiffPane.svelte': [
		...
		{ kind: 'del', oldLine: 19, newLine: null, text: '  let rows = $derived(file.hunks.flatMap((h) => h.lines));' },
		{ kind: 'add', oldLine: null, newLine: 19, text: '  // Rows come pre-flattened from Rust — no diff logic here.' },
		{ kind: 'add', oldLine: null, newLine: 20, text: '  let rows = $derived(file.rows);' },
		{ kind: 'add', oldLine: null, newLine: 21, text: '  let pinned = $derived(new Map(notes.map((n) => [n.line, n])));' },
		{ kind: 'ctx', oldLine: 20, newLine: 22, text: '' },
		{ kind: 'ctx', oldLine: 21, newLine: 23, text: '  function pin(line: number) {' },
		{ kind: 'add', oldLine: null, newLine: 24, text: '    dispatch("pin", { path: file.path, line });' },
	...
	'src/lib/components/FileTree.svelte': [
		...
		{ kind: 'del', oldLine: 8, newLine: null, text: '  <span class="dir">{group.dir}</span>' },
		{ kind: 'add', oldLine: null, newLine: 8, text: '  <span class="dir">{shortenDir(group.dir)}</span>' },
		{ kind: 'ctx', oldLine: 9, newLine: 9, text: '  {#each group.files as f (f.path)}' },
		{ kind: 'del', oldLine: 10, newLine: null, text: '    <FileRow {f} />' },
		{ kind: 'add', oldLine: null, newLine: 10, text: '    <FileRow {f} noteCount={counts[f.path] ?? 0} />' },
	...
	'src-tauri/Cargo.toml': [
		...
		{ kind: 'add', oldLine: null, newLine: 22, text: 'octocrab = "0.39"' },
		{ kind: 'add', oldLine: null, newLine: 23, text: 'keyring = "3.2"' },
